...
def fitness_func(player):
    return player.target_steps*(1+player.num_of_items*0.15)
...
def draw_result():
    ...
    stupids_a = str(round(stupids_avg,2))
    stupids_b = str(round(stupids_best,2))
    elites_a = str(round(elites_avg,2))
    elites_b = str(round(elites_best, 2))
...
def learning(who):
    global FPSCLOCK, DISPLAYSURF, BASICFONT1, BASICFONT2, BASICFONT3, maps, GENERATION_NUM, TESTMODE, ELITES_TEST, STUPIDS_TEST, GENERATION_MAX, average_steps_list, best_steps_list ,average_fitness_list, best_fitness_list
...
def testing(who):
    ...
    print("Mean of best steps : "+ str(np.mean(b_steps)))
    print("Mean of average steps : "+ str(np.mean(a_steps)))
