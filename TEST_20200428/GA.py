...
import numpy as np
import test_game_8angle as game_comp

def ordered_selection(dead_players, num_selection):
    player_fitness = []
    index_fitness = []
    selected_parents = []

    for index, player in enumerate(dead_players):
        player_fitness.append(player.fitness)
        index_fitness.append([index, player.fitness])

    if len(set(player_fitness)) == 1:
        player_fitness[np.random.randint(0, len(player_fitness))] += 1
        index_fitness[0][1] += 1

    index_fitness = sorted(index_fitness, reverse = True, key = lambda x: (x[1]))
    print(index_fitness)
    for i_p in index_fitness:
        selected_parents.append(i_p[0])
        if len(selected_parents) >= num_selection:
            break

    return selected_parents

def roulette_selection(dead_players, max_min_ratio, num_selection):
    player_fitness = []
    scaled_fitness = []
    selected_parents = []
    sum_of_fitness = 0
    ...
    if len(set(player_fitness)) == 1:
        player_fitness[np.random.randint(0, len(player_fitness))] += 1
        #player_fitness.append(player.steps + 10 * player.num_of_items)
    print(player_fitness)
...
def mutation(superior, prob_mutation, m, std): # random uniform mutation
    next_generation = []
    for bg in superior:
        new_genome = bg
